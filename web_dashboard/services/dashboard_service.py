...
def build_dashboard_summary():
    ...
    verified_count = sum(
        ...
        if bool(verified_users.get(username, False))
    )
    ...
    active_count = sum(
        1 for username in users
        if user_statuses.get(username, "active") == "active"
    )

    frozen_count = sum(
        1 for username in users
        if user_statuses.get(username) == "frozen"
    )

    banned_count = sum(
        1 for username in users
        if user_statuses.get(username) == "banned"
    )

    silver_count = sum(
        1 for username in users
        if user_plans.get(username) == "الباقة الفضية"
    )

    gold_count = sum(
        1 for username in users
        if user_plans.get(username) == "الباقة الذهبية"
    )

    vip_count = sum(
        1 for username in users
        if user_plans.get(username) == "باقة VIP"
    )
    ...
    total_capital = round(
        sum(float(user_deposits.get(username, 0)) for username in users),
        2
    )

    total_balances = round(
        sum(float(user_balance.get(username, 0)) for username in users),
        2
    )
